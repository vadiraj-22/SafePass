...
const LetterGlitch = ({
  glitchColors = ['#00ff66', '#34d399', '#10b981', '#22c55e', '#059669', '#00ff66', '#38bdf8', '#c084fc'],
  className = '',
  glitchSpeed = 50,
  ...
  const charWidth = Math.max(10, Math.round(fontSize * 0.8));
  const charHeight = Math.max(20, Math.round(fontSize * 1.5));
  ...
  const parseColor = (colorStr) => {
    if (!colorStr) return { r: 74, g: 246, b: 38 };
  ...
  const handleSmoothTransitions = () => {
    ...
    letters.current.forEach(letter => {
      if (letter.colorProgress < 1) {
        ...
        const startRgb = parseColor(letter.startColor || letter.color);
